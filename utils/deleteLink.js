...
+ */
const deleteLinkFunction =  (id_doctor) => async (id_user) => {
    // Finc doctor by id
    const doctor = await Doctor.findById( id_doctor );
    // If the doctor does not exist, return an error
    if (!doctor) {
        return { message: "Doctor non trouvé", status : 404 };
    }
    //Find user by id
    ...
    if (!doctor.users_link.includes(id_user)) {
        return { message: "Identifiant non valide Doctor", status : 400 };
    }
    // If the user haven't linked any doctor, return an error
    if (!user.doctors_link.includes(id_doctor)) {
        return { message: "Identifiant non valide User", status : 400 };
    }
    // If all is valide
    ...
    // return succes
    return { message: "Lien supprimé", status : 200 };
}
//////////
//////////
